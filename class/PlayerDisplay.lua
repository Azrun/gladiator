...
function _M:display()
	...
	--if player:knowTalent(player.T_STAMINA_POOL) then
		...
		self:mouseTooltip(self.TOOLTIP_POWER, self:makeTextureBar("#ffcc00#Power:", nil, player:getPower(), player.max_power, player.power_regen, x, h, 255, 255, 255, {r=0xff / 3, g=0xcc / 3, b=0x00 / 3}, {r=0xff / 6, g=0xcc / 6, b=0x00 / 6})) h = h + self.font_h
    
    h = h + self.font_h
	self:mouseTooltip(self.TOOLTIP_MAIN, self:makeTexture(("MAIN: #00ff00#%s"):format(player.action_queue.main_hand or "ERROR1"), x, h, 255, 255, 255)) h = h + self.font_h
    self:mouseTooltip(self.TOOLTIP_OFF, self:makeTexture(("OFF: #00ff00#%s"):format(player.action_queue.off_hand or "ERROR1"), x, h, 255, 255, 255)) h = h + self.font_h
    self:mouseTooltip(self.TOOLTIP_FEET, self:makeTexture(("FEET: #00ff00#%s"):format(player.action_queue.feet or "ERROR1"), x, h, 255, 255, 255)) h = h + self.font_h
